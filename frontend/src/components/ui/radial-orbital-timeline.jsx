...
import { ArrowRight, CalendarClock, Link } from "lucide-react";
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        <div className="flex items-center gap-1 mb-2">
                          <CalendarClock size={10} className="text-white/70" />
                          <span className="text-xs uppercase tracking-wider font-medium text-white/70">Deadline</span>
                        </div>
                        {item.daysLeft === null ? (
                          <p className="text-xs text-white/40 italic">No due date set</p>
                        ) : (
                          <>
                            <div className="flex justify-between items-center text-xs mb-1.5">
                              <span className="font-mono text-white/80">{item.dueLabel}</span>
                              <span className={`font-semibold ${
                                item.daysLeft < 0  ? "text-red-400"
                                : item.daysLeft === 0 ? "text-orange-400"
                                : item.daysLeft <= 3 ? "text-yellow-400"
                                : "text-green-400"
                              }`}>
                                {item.daysLeft < 0
                            ...
                            <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all ${
                                  ...
                                  : item.daysLeft <= 3 ? "bg-yellow-400"
                                  : item.daysLeft <= 7 ? "bg-blue-400"
                                  : "bg-green-500"
                                }`}
                                style={{ width: `${item.urgencyPct}%` }}
                              />
                            </div>
                          </>
                        )}
